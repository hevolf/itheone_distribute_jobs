...
public class QuartzConfig {
    ...
    * **********************************/
    ...
    public JobDetailFactoryBean businessJobDetail() {
        ...
        jobDetailFactoryBean.setJobClass(BusinessJob.class);//业务bean是多例的还是单例的？多例

        //将调度环境信息jobDataMap参数封装传递给job
    ...
    /*********************************
     * 触发器：trigger （需注入job）
     * 1. SimpleTriggerFactoryBean 简单触发器
     * 2. CronTriggerFactoryBean cron触发器
     * *******************************/
    // 简单触发器
    ...
    /**
     * 调度工厂,将所有的触发器引入（实例 simpleTrigger和cronTrigger）
     * @return
     */
    @Bean(name = "scheduler")
